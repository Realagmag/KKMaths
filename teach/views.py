...
def check_answer(request, answer_id):
    ...
    if request.user.is_authenticated:
        ...
        if not solved:
            Solved.objects.create(user=user, exercise=answer.exercise, correct=answer.correct)
        else:
...
def hard_exercises(request, exercise_id):
    if request.user.is_authenticated:
        if request.method == 'DELETE':
            try:
                struggle = Struggles.objects.get(student=request.user, exercise_id=exercise_id)
                struggle.delete()
            ...
            _, _ = Struggles.objects.get_or_create(student=request.user, exercise_id=exercise_id)
