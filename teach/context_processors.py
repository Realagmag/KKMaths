...
order = [
    ...
    "Trójkąty",
    "Kąty Środkowe i Kąty Wpisane",
    "Okrąg Opisany i Okrąg Wpisany",
...
def base_template_context(request):
    ...
    categories = (
        Category.objects.filter(name__in=order)
        .annotate(
            order=Case(
                *[
                    When(name=cat_name, then=order.index(cat_name))
    ...
    for category in categories:
        ...
        if all_exercises_count > 0:
            completed_exercises_in_category = category.exercise_set.filter(
                id__in=completed_exercises_ids
            ).count()
            completed_count_in_category[category.name] = completed_exercises_in_category
            completed_percent[category.name] = int(
                completed_exercises_in_category / all_exercises_count * 100
            )
        else:
...
def exercise_modification_context(request, exercise_id):
    ...
    completed_exercises_in_category = exercise.category.exercise_set.filter(
        id__in=completed_exercises_ids
    ).count()
    category_progress = int(
        completed_exercises_in_category / all_exercises_in_category * 100
    )
    return {
...
def calculate_total_progress(exercises, solved_exercises):
    ...
    for exercise in exercises:
        ...
        if solved_exercises.get(str(exercise.id)):
...
def get_completed_exercises_ids(solved_exercises):
    return [key for key, value in solved_exercises.items() if value]
